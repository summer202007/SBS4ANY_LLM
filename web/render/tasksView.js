...
function renderFeaturedDemo(task, context = {}) {
  ...
  const collected = runStats ? `${runStats.completed}/${runStats.total}` : hasTask ? task?.status === "setup" ? "Not started" : "Sample ready" : "13/15 sample";
  ...
  const verdict = report?.executiveVerdict?.verdict || taskVerdictFallback(task);
  const reason =
    ...
    <section class="featured-demo-card">
      ...
      <div class="featured-demo-grid">
        ...
        <div class="guided-flow" aria-label="Guided run flow">
          ...
          ${flowStep("Report", report || !hasTask ? "Ready" : "Pending", "SBS verdict + PDF")}
        </div>
      </div>
      <div class="summary-grid demo-context-grid">
        ...
        ${field("Automation Surface", "Eval set · capture assist · simulator · grader report")}
      </div>
...
function selectFeaturedTask(tasks, activeTaskId) {
  ...
  const active = tasks.find((task) => task.taskId === activeTaskId);
