...
## Optional: Install The Skills Into Codex / 可选：把 Skills 安装到 Codex

The macOS app and local workbench can use the repo-local skill files directly. You do not need to install these skills just to open and run SBS.

macOS App 和本地工作台可以直接使用仓库内置的 skill 文件。只想打开并运行 SBS，不需要额外安装这些 skills。

If you already use Codex and want the SBS workflows available in your own Codex sessions, install the repo skills from GitHub:

如果你已经在用 Codex，并希望在自己的 Codex 会话里直接调用 SBS 工作流，可以从 GitHub 安装本仓库的 skills：

```bash
npx -y skills add https://github.com/summer202007/SBS4ANY_LLM --full-depth --list
npx -y skills add https://github.com/summer202007/SBS4ANY_LLM -g -a '*' -s '*' --full-depth --copy -y
npx -y skills list -g --json
```

Included skills / 内置 skills：

- `chatbot-eval-set-generator`: turns a task-space idea into a structured runtime eval package.
- `chatbot-eval-set-generator`：把任务空间想法变成结构化 runtime eval package。
- `chatbot-runtime-user-simulator`: suggests side-blind next user turns for multi-turn chatbot evals.
- `chatbot-runtime-user-simulator`：为多轮 chatbot 评测生成 side-blind 下一轮用户消息。
- `chatbot-sbs-grader`: cleans evidence, grades cases, and produces SBS reports.
- `chatbot-sbs-grader`：清洗证据、裁判 cases，并生成 SBS 报告。
- `chatbot-website-capture-adapter-builder`: builds read-only capture adapter contracts for chatbot websites.
- `chatbot-website-capture-adapter-builder`：为 chatbot 网页构建只读采集适配器契约。

Flag notes / 参数说明：

- `--full-depth` scans nested `skills/*/SKILL.md` directories.
- `--full-depth` 会扫描嵌套的 `skills/*/SKILL.md` 目录。
- `-g` installs globally for your local agent environment.
- `-g` 会安装到你本机的全局 agent 环境。
- `-a '*' -s '*'` installs all repo skills for all supported agents.
- `-a '*' -s '*'` 会把仓库里的全部 skills 安装给所有支持的 agents。
- `--copy` copies the skill files instead of linking to a temporary checkout.
- `--copy` 会复制 skill 文件，而不是链接到临时 checkout。
